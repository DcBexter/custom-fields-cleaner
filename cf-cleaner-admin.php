...
/* Save alternative prefixes as a cookie (don't mesh the database...) */
  ...
  <form method="post" action="#" onsubmit="cf_prefix_save();">
    ...
    <table class="form-table">
      ...
      <td>
          <label for="cf_filter"></label><select style="min-width:160px;" name="cf_filter" id="cf_filter">
          <option value="3"<?php if ($cf_filter == '3' || $cf_filter == '0') { echo ' selected'; } ?>><?php _e('Clean all', 'cf_cleaner'); ?></option>
          <option value="1"<?php if ($cf_filter == '1') { echo ' selected'; } ?>><?php _e('Orphans only', 'cf_cleaner'); ?></option>
          <option value="2"<?php if ($cf_filter == '2') { echo ' selected'; } ?>><?php _e('Empty only', 'cf_cleaner'); ?></option>
        </select>
      </td>
      </tr>
      ...
      <td><label for="cf_db_prefix"></label><input type="text" name="cf_db_prefix" id="cf_db_prefix" value="<?php echo $cf_db_prefix; ?>" placeholder="wp_" style="min-width:160px;"></td>
      </tr>

      <tr>
      <th scope="row"><?php _e('Field prefix (at least 3 chrs.) :', 'cf_cleaner'); ?></th>
      <td><label for="cf_prefix"></label><input type="text" name="cf_prefix" id="cf_prefix" value="<?php echo $cf_prefix; ?>" placeholder="cf_" style="min-width:160px;"></td>
      </tr>
    ...
    <p>
      <button data-action="cf-clean" class="button button-primary" type="submit"><?php _e('CF <b>clean</b>', 'cf_cleaner'); ?></button>
      <button data-action="cf-check" class="button button-secondary" type="button" onclick="cf_prefix_save();window.location = location.href;"><?php _e('CF <b>check</b>', 'cf_cleaner'); ?></button>
    </p>
